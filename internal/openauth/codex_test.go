...
func TestDeviceLoginUsesFixedFlowAndStoresCredential(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/token":
			...
			json.NewEncoder(response).Encode(oauthTokenResponse{AccessToken: "access-token-123", RefreshToken: "refresh-token-123", IDToken: idToken, ExpiresIn: 3600})
		default:
...
func TestManagerRefreshesExpiredCredentialAndPersistsRotation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		json.NewEncoder(response).Encode(oauthTokenResponse{AccessToken: "new-access-token", RefreshToken: "new-refresh-token", IDToken: idToken, ExpiresIn: 1800})
	}))
	defer server.Close()
