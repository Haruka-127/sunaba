...
- guest OpenCode sessionがcustom `sunaba` providerを使い、base URLと短命tokenだけで固定Model GatewayへResponses requestを送り、host側mock upstreamのstreaming応答を受け取った。upstreamにはhost側実credentialだけが届いた。
