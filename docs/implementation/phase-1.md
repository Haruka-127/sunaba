...
- canonical OS temp直下の短いmode `0700` runtime base（`sunaba-d-*` / `sunaba-s-*`）へ、VM-boundな`sunaba-vm-*` rootを作る。Model/Git/Web/attach、approval control、最大長remote名のGit hookを含む全Unix socket pathは、target OSの`sockaddr_un`から導出した上限内であることをbind/dial前に検証する。旧`dev-recovery-*` / `sunaba-recovery-*` runtimeは既存recordまたはstale locatorのexact identity contractが一致する場合だけ復旧・明示破棄できる。Host TUIのHOME/XDG stateには、その配下へAgent Sessionごとに一意な`sunaba-session-*` rootを作り、TUI終了時に削除する。永続stateとlockはStore側に分離する。
